...
### Solidtime
- Time tracking application at `app.example.com`
...
- URL: `https://start.exactonline.nl/docs/ProTimeImport.aspx?_Division_=0000000`
...
- Hour types (Uursoort) used:
  - `HOURS-EXT-PROJ` - External consulting project work
  - `HOURS-EXT-MEET` - External consulting meetings
  - `HOURS-INT-PROJ` - Internal consulting project work
  - `HOURS-INT-MEET` - Internal consulting meetings
  - `HOURS-PTO` - Vacation/PTO
  - `HOURS-SICK` - Sick leave/absence
  - `TRAINING` - Training hours
  - `NON BILLABLE` - Non-billable internal work

### Project Naming Conventions
Format: `{CLIENT}-{TYPE}`
- Clients: abbreviated by initials, e.g. ACME for Acme Corp
...
- Examples from projects.json:
  - `Acme-PTO` → Vacation hours (HOURS-PTO)
  - `Acme-Sick` → Sick leave (HOURS-SICK)
  - `Example-Meetings` → External client meeting hours (HOURS-EXT-MEET)
  - `Acme-Meetings` → Internal meeting hours (HOURS-INT-MEET)
  - `Example-Project` → External project hours (HOURS-EXT-PROJ)
...
./prepareexact.py --open-url https://start.exactonline.nl/docs/ProTimeImport.aspx?_Division_=0000000
```
...
- **Division ID**: Exact division (0000000) is hardcoded in URLs
