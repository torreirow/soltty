...
Soltty is a CLI tool for Solidtime time tracking, built with Go and Cobra. Users configure the tool via `config.json` which includes an API `base_url` field (e.g., `https://app.example.com/api/v1`). The system needs to add a `web` command that opens the Solidtime web interface in the user's default browser.
...
**Rationale:**
- The `base_url` already contains the full API endpoint (e.g., `https://app.example.com/api/v1`)
- The web interface is typically at the root of the same domain (e.g., `https://app.example.com`)
